...
namespace msc
{
	                                  ...
	                                  float minLife, float maxLife, SpriteID spriteID, uint32_t numParticles)
	{
		...
		{
			numParticles = mMaxParticles - static_cast<uint32_t>(mSpriteInstances.size());
			// Will ignore excess particles
		}
	...
	bool ParticleSystem::Update(float frameTime)
	{
		...
		while (spriteInstance != spriteInstanceEnd)
		{
			...
			spriteInstance->position.head<2>() += updateData->velocity * frameTime;
			// the head<2> accesses the x & y of a Vector3f as a Vector2f
