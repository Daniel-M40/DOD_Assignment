...
EngineDX::EngineDX(platform::Window& window, bool fullscreen /*= false*/) 
{
...
#endif
	const D3D_FEATURE_LEVEL featureLevels[] = { D3D_FEATURE_LEVEL_11_1, D3D_FEATURE_LEVEL_11_0 }; // Want DirectX 11
  ...
  // GPU - but how might we determine which is the best GPU?
	hr = D3D11CreateDevice(NULL, D3D_DRIVER_TYPE_HARDWARE, NULL, flags,    // Note: if you do select a specific adapter rather than NULL then must use D3D_DRIVER_TYPE_UNKNOWN
  ...
  if (!mIsFullscreen)
	{
		swapDesc.BufferDesc.Width  = mWindowedSize.x(); // For windowed, just change dimensions from monitor's initial mode
  ...
  mDevice->CreateRenderTargetView(mBackBufferTexture, NULL, &mBackBufferRTV);
  ...
  // Set viewport as entire back buffer
	...
	viewport.TopLeftX = 0;
	viewport.TopLeftY = 0;
	viewport.Width  = (FLOAT)mBackBufferSize.x();
	viewport.Height = (FLOAT)mBackBufferSize.y();
...
bool MonitorModeOrder(const DXGI_MODE_DESC& a, const DXGI_MODE_DESC& b)
{
  ...
  if (a.Scaling == DXGI_MODE_SCALING_STRETCHED && b.Scaling != DXGI_MODE_SCALING_STRETCHED) return false;
  if (b.Scaling == DXGI_MODE_SCALING_STRETCHED && a.Scaling != DXGI_MODE_SCALING_STRETCHED) return true;
  ...
  if (formatSizeA < formatSizeB)  return false;
  if (formatSizeA > formatSizeB)  return true;
  ...
  if (refreshRateA < refreshRateB)  return false;
  if (refreshRateA > refreshRateB)  return true;

  if (a.ScanlineOrdering == DXGI_MODE_SCANLINE_ORDER_PROGRESSIVE && b.ScanlineOrdering != DXGI_MODE_SCANLINE_ORDER_PROGRESSIVE) return true;
  if (b.ScanlineOrdering == DXGI_MODE_SCANLINE_ORDER_PROGRESSIVE && a.ScanlineOrdering != DXGI_MODE_SCANLINE_ORDER_PROGRESSIVE) return false;
...
void EngineDX::CollectAdapterInfo()
{
  ...
  { 
    ...
    if (hr != S_OK)  continue; // Error, skip this adapter
    ...
    { 
      ...
      if (hr != S_OK)  continue; // Error, skip this monitor

      // Get display modes supported by this monitor
      UINT numModes = 0;
      DXGI_FORMAT format = DXGI_FORMAT_R8G8B8A8_UNORM; // Only look for standard colour depth - integer RGBA 8-bits each
      UINT flags         = DXGI_ENUM_MODES_SCALING;    // Will include non-native resolutions that require scaling
      do
      {
        if (monitor.dxgi->GetDisplayModeList(format, flags, &numModes, 0) != S_OK) break;
      ...
      if (hr != S_OK) monitor.modes.clear(); // List the monitor, but no modes on error
...
bool EngineDX::GetCurrentMonitorMode(const Monitor& monitor, DXGI_MODE_DESC* outDesc, bool usePreferredRefreshRate /*= true*/) const
{
  // Get details about given monitor
  HDC hdcMonitor = CreateDCW( monitor.desc.DeviceName, monitor.desc.DeviceName, NULL, NULL );
  if (hdcMonitor == NULL)  return false;
  ...
  return (monitor.dxgi->FindClosestMatchingMode( &searchDesc, outDesc, NULL ) == S_OK);
}
...
void EngineDX::CreatePipelineStateObjects()
{
  ...
  // "example" vertex shader to validate the layout, which is why mSpriteVS is used
	mDevice->CreateInputLayout(SpriteRenderDataLayout, sizeof(SpriteRenderDataLayout)/sizeof(SpriteRenderDataLayout[0]),
  ...
  mContext->OMSetBlendState(mBlendNone, 0, 0xffffffff); // Default to no blending
  ...
  mContext->PSSetSamplers(0, 1, &mBilinearNoMipsSampler.p); // Set bilinear sampler straight away on pixel shader for texture 0
	...
	mContext->GSSetConstantBuffers(0, 1, &mPerSpriteSetCB.p); // Set constant buffer straight away on geometry shader buffer 0
...
void EngineDX::SetRenderMode(RenderMode renderMode)
{
  ...
  if (renderMode == RenderMode::Cutout)
  {
    ...
    mContext->OMSetBlendState(mBlendNone, 0, 0xffffffff);
  }
  else if (renderMode == RenderMode::Additive)
  {
	  mContext->OMSetDepthStencilState(mDepthWritesDisable, 0);
    mContext->OMSetBlendState(mBlendAdditive, 0, 0xffffffff);
  }
  ...
  {
	  mContext->OMSetDepthStencilState(mDepthWritesDisable, 0);
    mContext->OMSetBlendState(mBlendAlpha, 0, 0xffffffff);
  }
}
