...
private:
  ...
  bool GetCurrentMonitorMode(const Monitor& monitor, DXGI_MODE_DESC* outDesc, bool usePreferredRefreshRate = true) const;
  ...
  // Window related members (including DXGI)
 	//-------------------------------------------------------------------------------------------------
private:
	CComPtr<IDXGIFactory> mDXGIFactory; // Access to DXGI (used to create swap chain, but also handles attached graphics cards, supported resolutions etc.)
...
private:
  ...
  CComPtr<ID3D11DepthStencilView> mDepthStencilView;
	Vector2i mBackBufferSize;  // Doesn't have to match window size, but final render will be scaled before presentation if it doesn't
...
private:
  ...
  // First, a structure to hold the data CPU-side, then a DirectX object for the GPU-side buffer
	...
	struct 
	{
		float backbufferSize[2]; // Not using eigen math library types here as they might conflict with packing requrements
    float atlasSize[2];
