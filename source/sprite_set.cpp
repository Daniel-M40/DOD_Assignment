...
namespace msc
{
	...
	SpriteID SpriteSet::DefineSprite(std::string filename, SpriteAnchor anchor, float anchorX /*= 0.5f*/,
	                                 float anchorY /*= 0.5f*/)
	{
		if (mAtlasCreated)
		{
			return INVALID_ID; // Can't add more sprites after Finalise has been called
		}
		...
		try
		{
			...
			if (anchor == SpriteAnchor::Centre)
			{
				newSprite.anchor = {width * 0.5f, height * 0.5f};
			}
			else if (anchor == SpriteAnchor::TopLeft)
			{
				newSprite.anchor = {0, 0};
			}
			else if (anchor == SpriteAnchor::TopRight)
			{
				newSprite.anchor = {width, 0};
			}
			else if (anchor == SpriteAnchor::BottomLeft)
			{
				newSprite.anchor = {0, height};
			}
			else if (anchor == SpriteAnchor::BottomRight)
			{
				newSprite.anchor = {width, height};
			}
			else
			{
				newSprite.anchor = {anchorX, anchorY};
			}

			newSprite.filename = filename; // Not loading file data until the Finalise method, so store filename now
			newSprite.ID = static_cast<uint32_t>(mSprites.size());
			// The new sprite's ID refers to end of current sprite list...
	...
	bool SpriteSet::Finalise()
	{
		if (mAtlasCreated)
		{
			return false; // Can only finalise once
		}
	...
	bool SpriteSet::Load(uint32_t maxInstances)
	{
		if (mLoaded)
		{
			if (maxInstances != mMaxInstances)
			{
				Unload();
				// Strange case of user calling Load again with different maxInstances (always check your corners...)
			}
			else
			{
				return true; // Already loaded (note that this is not a failure, so return true)
			}
		}
		...
		mInstancesToRender = std::make_unique<SpriteRenderData[]>(mMaxInstances);
		// Note that this array version of make_unique is from C++14
		...
		if (FAILED(mDevice->CreateBuffer(&bufferDesc, nullptr, &mVertexBuffer)))
		{
			return false;
		}
	...
	void SpriteSet::AddToRenderQueue(SpriteInstance* instances, uint32_t number, Vector2f offset /*= {0, 0}*/)
	{
		...
		{
			number = mMaxInstances - static_cast<uint32_t>((mInstancesToRenderEnd - mInstancesToRender.get()));
			// Will ignore excess instances
		}
	...
	bool SpriteSet::Render()
	{
		...
		if (FAILED(mContext->Map(mVertexBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &bufferData)))
		{
			return false;
		}
	...
	bool SpriteSet::CreateSpriteAtlas()
	{
		...
		{
			return false;
			// Should this be true or false...?  Zero area atlas could cause problems elsewhere so go for false.
		}
		...
		do
		{
			...
			rectangles.reserve(mSprites.size() * 2 + 1);
			// Reserve space for maximum number of rectangles generated by the algorithm below
			...
			mAtlasSize = {0, 0};
			// Reset the atlas size now, and track the actual extent of the used area when adding each sprite
			do
			{
				...
				while (sprite != mSprites.end() && (sprite->UV.x() >= 0 ||
					sprite->size.x() > currentRectangle->size.x() || sprite->size.y() > currentRectangle->size.y()))
				{
					++sprite;
				}
				...
				else
				{
					...
					sprite->UV.x() = static_cast<float>(currentRectangle->position.x());
					sprite->UV.y() = static_cast<float>(currentRectangle->position.y());
					sprite->atlasIndex = 0;
					// @todo for very large sprite sets that need an array of atlases, not implemented

					// Track the maximum used x and y coordinate to determine the final size for the atlas
					if (sprite->UV.x() + sprite->size.x() > mAtlasSize.x())
					{
						mAtlasSize.x() = static_cast<uint32_t>(sprite->UV.x() + sprite->size.x());
					}
					if (sprite->UV.y() + sprite->size.y() > mAtlasSize.y())
					{
						mAtlasSize.y() = static_cast<uint32_t>(sprite->UV.y() + sprite->size.y());
					}

					// Update total area fitted to see if we have finished
					totalAreaFitted += static_cast<uint32_t>(sprite->size.x()) * static_cast<uint32_t>(sprite->size.
						y());
					if (totalAreaFitted == mTotalSpriteArea)
					{
						goto LOAD_SPRITES; // YES! I used goto!!!
					}
					...
					if (sprite->size.x() < currentRectangle->size.x())
					{
						if (sprite->size.y() < currentRectangle->size.y())
						{
							...
							belowPosition.y() += static_cast<uint32_t>(sprite->size.y());
							belowSize.y() -= static_cast<uint32_t>(sprite->size.y());
							rectangles.push_back(AtlasRectangle{belowPosition, belowSize});
						}
						// Update current rectangle to the empty area to the right
						currentRectangle->position.x() += static_cast<uint32_t>(sprite->size.x());
						currentRectangle->size.x() -= static_cast<uint32_t>(sprite->size.x());
						currentRectangle->size.y() = static_cast<uint32_t>(sprite->size.y());
					}
					else if (sprite->size.y() < currentRectangle->size.y())
					{
						...
						currentRectangle->position.y() += static_cast<uint32_t>(sprite->size.y());
						currentRectangle->size.y() -= static_cast<uint32_t>(sprite->size.x());
						currentRectangle->size.x() = static_cast<uint32_t>(sprite->size.x());
					}
					else
					{
	...
	LOAD_SPRITES:
		...
		try
		{
			...
			for (auto& sprite : mSprites)
			{
				...
				uint32_t offsetX = static_cast<uint32_t>(sprite.UV.x());
				uint32_t offsetY = static_cast<uint32_t>(sprite.UV.y());
				sprite.UV.x() = (sprite.UV.x() + 0.5f) / mAtlasSize.x();
				// Add half a pixel to the top/left to stop texture
				sprite.UV.y() = (sprite.UV.y() + 0.5f) / mAtlasSize.y();
				// bleeding into neigbouring atlas sprites. See geometry shader

				// Load and copy pixel data
				CImg<uint8_t> spriteImage(sprite.filename.c_str());
				uint32_t spriteWidth = static_cast<uint32_t>(sprite.size.x());
				uint32_t spriteHeight = static_cast<uint32_t>(sprite.size.y());
				...
				for (uint32_t x = 0; x < spriteWidth; ++x)
				{
					for (uint32_t y = 0; y < spriteHeight; ++y)
					{
						...
						mAtlasCPU(offsetX + x, offsetY + y, 3) = (hasAlpha ? spriteImage(x, y, 3) : 255);
						// Sprite might have alpha or not. The atlas always does
					}
				}
			}
			return true;
